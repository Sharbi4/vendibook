/**
 * GET/POST /api/metrics/me - Fetch metrics for current user via Clerk ID
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [record] = await sql`SELECT * FROM user_metrics WHERE user_id = ${user.id} LIMIT 1`;

    if (!record) {
      return res.status(200).json({
        success: true,
        data: null,
        message: 'No metrics profile yet. POST to /api/metrics to initialize metrics.',
        userId: user.id
      });
    }
    ...
    console.error('Failed to fetch metrics for current user:', error);
    return res.status(500).json({
      success: false,
      error: 'Server error',
